...
loop:
    ...
    abs $t4, $t4         # Take the absolute value of the current element
    bgt $t4, $t2, update  # If the absolute value is greater than the maximum absolute value, update the maximum
    addi $t0, $t0, 4     # Move to the next element
    addi $t1, $t1, -1    # Decrease the size
    j loop

update:
    move $t2, $t4        # Update the maximum absolute value
